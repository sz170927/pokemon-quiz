...
const SAMPLE_POKEMON = [
  { dex:25, korean_name:"피카츄", english_name:"Pikachu", types:["전기"], base_stats:{hp:35, atk:55, def:40, spa:50, spd:50, spe:90}, image_url:"" },
  { dex:1, korean_name:"이상해씨", english_name:"Bulbasaur", types:["풀","독"], base_stats:{hp:45, atk:49, def:49, spa:65, spd:65, spe:45}, image_url:"" },
  { dex:4, korean_name:"파이리", english_name:"Charmander", types:["불꽃"], base_stats:{hp:39, atk:52, def:43, spa:60, spd:50, spe:65}, image_url:"" },
  { dex:7, korean_name:"꼬부기", english_name:"Squirtle", types:["물"], base_stats:{hp:44, atk:48, def:65, spa:50, spd:64, spe:43}, image_url:"" },
  { dex:130, korean_name:"갸라도스", english_name:"Gyarados", types:["물","비행"], base_stats:{hp:95, atk:125, def:79, spa:60, spd:100, spe:81}, image_url:"" },
  { dex:94, korean_name:"팬텀", english_name:"Gengar", types:["고스트","독"], base_stats:{hp:60, atk:65, def:60, spa:130, spd:75, spe:110}, image_url:"" },
  { dex:149, korean_name:"망나뇽", english_name:"Dragonite", types:["드래곤","비행"], base_stats:{hp:91, atk:134, def:95, spa:100, spd:100, spe:80}, image_url:"" },
  { dex:133, korean_name:"이브이", english_name:"Eevee", types:["노말"], base_stats:{hp:55, atk:55, def:50, spa:45, spd:65, spe:55}, image_url:"" },
  { dex:448, korean_name:"루카리오", english_name:"Lucario", types:["격투","강철"], base_stats:{hp:70, atk:110, def:70, spa:115, spd:70, spe:90}, image_url:"" },
  { dex:6, korean_name:"리자몽", english_name:"Charizard", types:["불꽃","비행"], base_stats:{hp:78, atk:84, def:78, spa:109, spd:85, spe:100}, image_url:"" },

  // --- 추가 세트 ---
  { dex:430, korean_name:"돈크로우", english_name:"Honchkrow", types:["악","비행"], base_stats:{hp:100, atk:125, def:52, spa:105, spd:52, spe:71}, image_url:"" },
  { dex:229, korean_name:"헬가", english_name:"Houndoom", types:["악","불꽃"], base_stats:{hp:75, atk:90, def:50, spa:110, spd:80, spe:95}, image_url:"" },
  { dex:19, korean_name:"꼬렛", english_name:"Rattata", types:["노말"], base_stats:{hp:30, atk:56, def:35, spa:25, spd:35, spe:72}, image_url:"" },
  { dex:381, korean_name:"라티오스", english_name:"Latios", types:["드래곤","에스퍼"], base_stats:{hp:80, atk:90, def:80, spa:130, spd:110, spe:110}, image_url:"" },
  { dex:380, korean_name:"라티아스", english_name:"Latias", types:["드래곤","에스퍼"], base_stats:{hp:80, atk:80, def:90, spa:110, spd:130, spe:110}, image_url:"" },
  { dex:95, korean_name:"롱스톤", english_name:"Onix", types:["바위","땅"], base_stats:{hp:35, atk:45, def:160, spa:30, spd:45, spe:70}, image_url:"" },
  { dex:157, korean_name:"블레이범", english_name:"Typhlosion", types:["불꽃"], base_stats:{hp:78, atk:84, def:78, spa:109, spd:85, spe:100}, image_url:"" },
  { dex:823, korean_name:"아머까오", english_name:"Corviknight", types:["비행","강철"], base_stats:{hp:98, atk:87, def:105, spa:53, spd:85, spe:67}, image_url:"" },
  { dex:658, korean_name:"개굴닌자", english_name:"Greninja", types:["물","악"], base_stats:{hp:72, atk:95, def:67, spa:103, spd:71, spe:122}, image_url:"" },
  { dex:121, korean_name:"아쿠스타", english_name:"Starmie", types:["물","에스퍼"], base_stats:{hp:60, atk:75, def:85, spa:100, spd:85, spe:115}, image_url:"" },
  { dex:217, korean_name:"링곰", english_name:"Ursaring", types:["노말"], base_stats:{hp:90, atk:130, def:75, spa:75, spd:75, spe:55}, image_url:"" },
];
...
// ===== 난이도/채점 =====
const DIFFICULTY = { timePerQuestionSec: 15, wrongPenalty: -2, correctReward: 5, statExactOnly: true };
...
function Badge({children}){ return <span className="inline-block px-2 py-0.5 text-xs rounded-full border bg-white/70">{children}</span>; }
function StatRow({label, value}){ return (<div className="flex items-center justify-between text-sm"><span className="text-gray-600">{label}</span><span className="font-mono">{value}</span></div>); }
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    timerRef.current = setInterval(()=>{
      setTimeLeft(t=>{
        if(t<=1){ handleSubmit("TIMEOUT"); return DIFFICULTY.timePerQuestionSec; }
        return t-1;
      });
    },1000);
  ...
  function resetGame(){
    setScore(0); setLives(3); setQIndex(0);
    setAnswer(""); setTypeAnswers(["",""]);
  ...
  function finishIfDead(nextLives){
    if(nextLives<=0){
      setRunning(false);
      setHistory(h=>[{ ts:Date.now(), mode, score }, ...h].slice(0,50));
    }
  }
  function handleSubmit(){
    ...
    if(mode==="choseong"){
      const user=normalizeKor(answer), truth=normalizeKor(current.korean_name);
      correct=(user===truth);
    }
    if(mode==="types"){
      const userTypes=typeAnswers.map(t=>t.trim()).filter(Boolean);
      const truth=current.types;
      correct=(userTypes.length===truth.length)&&userTypes.every(t=>truth.includes(t));
    }
    if(mode==="stats"){
    ...
    if(correct){
      ...
      setLives(l=>{const next=l-1; finishIfDead(next); return next;});
      setQIndex(i=>i+1);
    }
    setAnswer(""); setTypeAnswers(["",""]);
    setStatAnswers({hp:"", atk:"", def:"", spa:"", spd:"", spe:""});
  ...
  function normalizeKor(s){ return s.replace(/\s+/g,"").trim(); }

  function importJson(){
    const raw = prompt("포켓몬 JSON 배열을 붙여넣으세요 (덮어쓰기). 예시는 i 버튼 참고.");
    if(!raw) return;
    try{
  ...
  function exportJson(){
    ...
    const a = document.createElement('a'); a.href=url; a.download='pokemon_quiz_data.json';
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-sky-50 to-indigo-50 text-gray-900 p-4 md:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={()=>{
                ...
                localStorage.removeItem("pokemon_quiz_data_v1");
                localStorage.removeItem("pokemon_quiz_scores_v1");
                location.reload();
              }}
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <Panel title="모드">
            ...
            <div className="mt-3 text-sm text-gray-600">
              ⏱️ 문항당 {DIFFICULTY.timePerQuestionSec}초 • ✅ +{DIFFICULTY.correctReward} • ❌ {DIFFICULTY.wrongPenalty}
            </div>
          </Panel>

          <Panel title="진행 상태" right={<Badge>매우 어려움</Badge>}>
            <div className="grid grid-cols-3 gap-2 text-center">
              <div><div className="text-xs text-gray-600">점수</div><div className="text-2xl font-bold">{score}</div></div>
              <div><div className="text-xs text-gray-600">목숨</div><div className="text-2xl font-bold">{"❤".repeat(Math.max(lives,0)) || "✖"}</div></div>
              <div><div className="text-xs text-gray-600">남은시간</div><div className={`text-2xl font-bold ${timeLeft<=5?"text-red-600":""}`}>{running? timeLeft : "-"}</div></div>
            </div>
        ...
        <Panel title="현재 포켓몬 정보 (학습/검증용 — 실제 라운드 중엔 보지 마세요!)">
          {current && (
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">{current.types.map((t,i)=>(<Badge key={i}>{t}</Badge>))}</div>
        ...
        <Panel title="데이터 관리 / 규격">
          <p className="text-sm text-gray-700 leading-relaxed">
            ...
            <span className="text-gray-500">타입은 한국어 표기로 입력하세요. 정답 판정은 순서 무시 일치입니다.</span>
          </p>
        </Panel>
...
function ModeButton({label, active, onClick}){
  return <button onClick={onClick} className={`px-3 py-1.5 rounded-xl border ${active?"bg-indigo-600 text-white":"bg-white"}`}>{label}</button>;
}
    ...
    <div className="space-y-4">
      ...
      {mode==='choseong' && (
        <div className="space-y-3">
          ...
          <input autoFocus value={answer} onChange={e=>setAnswer(e.target.value)} placeholder="정답(한국어 전체 이름)" className="w-full px-3 py-2 rounded-xl border" />
      ...
      {mode==='types' && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 max-w-xl">
            {[0,1].map(i=>(
              <input key={i} value={typeAnswers[i]} onChange={e=>{const next=[...typeAnswers]; next[i]=e.target.value; setTypeAnswers(next);}}
                placeholder={i===0?"1타입 (예: 전기)":"2타입 (없으면 비워두기)"} list="type-list" className="px-3 py-2 rounded-xl border" />
            ))}
          </div>
          <datalist id="type-list">{KOR_TYPES.map(t=>(<option key={t} value={t}/>))}</datalist>
          <div className="text-xs text-gray-500">순서 무시, 정확 일치</div>
        </div>
      )}
      ...
      {mode==='stats' && (
        <div className="space-y-3">
          ...
          <div className="text-xs text-gray-500">여섯 스탯 완전 일치만 정답</div>
        </div>
      )}
    </div>
  );
  ...
  return (
    <label className="text-sm">
      ...
      <input inputMode="numeric" pattern="[0-9]*" value={value}
        ...
        placeholder="숫자" className="w-full px-3 py-2 rounded-xl border" />
    </label>
  );
}
    ...
    <div className="space-y-2 max-h-56 overflow-auto pr-1">
      {history.map((h,i)=>(
        <div key={i} className="flex items-center justify-between text-sm border rounded-xl px-3 py-2 bg-white/60">
          ...
          <div className="flex items-center gap-2"><Badge>{h.mode==='choseong'?'초성':h.mode==='types'?'타입':'종족값'}</Badge><span className="font-semibold">{h.score}</span></div>
        </div>
      ))}
    </div>
  );
...
`}</pre>
          ...
          <div className="mt-1">• 종족값 모드는 여섯 스탯이 모두 정확해야 정답.</div>
